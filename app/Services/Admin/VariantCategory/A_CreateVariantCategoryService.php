...
class A_CreateVariantCategoryService
{
    /**
    ...
    public function create(array $data): JsonResponse
    {
        ...
        try {
            ...
            $variant = VariantCategory::create(Arr::except($data, ['localized']));

            foreach($data['localized'] as $local){
                VariantCategoryLocale::create([
            ...
            \DB::commit();
            return response()->json(successResponse(message: trans(SuccessMessagesEnum::CREATED), data: A_ShortVariantResource::make($variant)));
        } catch (\Exception $ex) {
            \DB::rollBack();
